...
def synth(text: str) -> tuple[np.ndarray, int]:
    """Synthesize text, preferring piper over espeak-ng.

    Returns (int16 audio, sample_rate).
    """
    if os.path.exists(_PIPER_MODEL):
        ...
        if proc.returncode == 0 and proc.stdout:
            return np.frombuffer(proc.stdout, dtype=np.int16), 22050
    proc = subprocess.run(
    ...
    if sampwidth != 2:
        raise RuntimeError(f"espeak-ng unexpected sample width: {sampwidth}")
...
def play(audio: np.ndarray, source_rate: int = 22050):
    ...
    # Polyphase resample to 48kHz with an anti-aliasing filter so we don't get
    # zero-order-hold staircase artifacts from nearest-neighbor indexing.
    audio_f = audio.astype(np.float32) / 32768.0
    if source_rate != JACK_RATE:
        g = np.gcd(int(JACK_RATE), int(source_rate))
        up = int(JACK_RATE) // g
        down = int(source_rate) // g
        resampled = resample_poly(audio_f, up, down).astype(np.float32)
    else:
        resampled = audio_f
